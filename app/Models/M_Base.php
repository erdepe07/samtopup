...
class M_Base extends Model
{
	...
	public function u_get($key)
	{
		$query = $this->db->table('utility')->where('u_key', $key)->get();
		$row = $query->getRow(); // Mengambil satu baris saja

		if ($row) {
			return $row->u_value;
		} else {
			// Jika key tidak ditemukan, kembalikan null atau pesan error agar tidak crash
			return null; 
		}
	}
